...
class Parser:
    ...
    def parse_declaration(self) -> Node:
        ...
        if self._check("COHERENT") or self._check("KEYWORD", "object"):
    ...
    def parse_function(self) -> FunctionNode:
        ...
        self.push_scope()
        for p in params:
            self.scopes[-1][p.name] = True
        self._expect("{")
        body = self.parse_block()
        self.pop_scope()
        return FunctionNode(name, params, ret_type, body)

    def parse_param(self) -> ParamNode:
        name = self._expect("IDENTIFIER").value
        self._expect(":")
        typ = self.parse_type()
        return ParamNode(name, typ)
    ...
    def parse_type(self) -> TypeNode:
        ...
        if tok.type == "IDENTIFIER":
            self._advance()
            return TypeNode(tok.value)
        raise SyntaxError(f"Expected type at line {tok.line}")

    def parse_object(self) -> ObjectNode:
        coherent = False
        if self._match("COHERENT"):
            coherent = True
        self._expect("KEYWORD", "object")
        name = self._expect("IDENTIFIER").value
        self._expect("{")
        members: List[Node] = []
        while not self._check("}"):
            if self._match_keyword("function"):
                members.append(self.parse_method())
            else:
                members.append(self.parse_field())
        self._expect("}")
        return ObjectNode(name, coherent, members)
    ...
    def parse_method(self) -> MethodNode:
        ...
        body = self.parse_block()
        self.pop_scope()
        return MethodNode(name, params, ret_type, body)
